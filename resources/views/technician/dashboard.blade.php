...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-orange-600 to-red-700 rounded-2xl p-6 text-white">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold">Welcome, {{ $technician->name ?? 'Technician' }}!</h1>
                <p class="text-orange-100 mt-1">Role: {{ ucfirst($technician->role ?? 'Technician') }} • Area: {{ $technician->area ?? 'All Areas' }}</p>
            </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <a href="{{ route('technician.tasks') }}" class="bg-white rounded-xl shadow-sm p-4 border border-gray-100 hover:shadow-md transition text-center">
            ...
            <p class="font-medium text-gray-800">My Tasks</p>
        </a>
        <a href="{{ route('technician.installations') }}" class="bg-white rounded-xl shadow-sm p-4 border border-gray-100 hover:shadow-md transition text-center">
            ...
            <p class="font-medium text-gray-800">Installations</p>
        </a>
        <a href="{{ route('technician.repairs') }}" class="bg-white rounded-xl shadow-sm p-4 border border-gray-100 hover:shadow-md transition text-center">
            ...
            <p class="font-medium text-gray-800">Repairs</p>
        </a>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
        <div class="p-6 border-b border-gray-100 flex justify-between items-center">
            ...
            <a href="{{ route('technician.tasks') }}" class="text-orange-600 hover:text-orange-700 text-sm">View All</a>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-lg font-semibold text-gray-800">Network Map</h2>
            <a href="{{ route('technician.map') }}" class="text-orange-600 hover:text-orange-700 text-sm">Open Map</a>
        </div>
        <div class="bg-gray-100 rounded-lg h-48 flex items-center justify-center">
            <div class="text-center text-gray-500">
                <i class="fas fa-map-marked-alt text-4xl mb-2"></i>
                <p>Click to view ODP & Customer map</p>
            </div>
        </div>
    </div>
</div>
